...
public class Aseprite : Aseprite.IUserDataTarget
{
	...
	public class Layer : IUserDataTarget
	{
		...
		public bool LockMovement => Flags.HasFlag(LayerFlags.LockMovement);
		public bool Background => Flags.HasFlag(LayerFlags.Background);
		public bool PreferLinkedCels => Flags.HasFlag(LayerFlags.PreferLinkedCels);
		public bool DisplayCollapsed => Flags.HasFlag(LayerFlags.DisplayCollapsed);
		public bool Reference => Flags.HasFlag(LayerFlags.Reference);
	...
	private void Load(BinaryReader bin)
	{
		...
		if (ReadWord() != 0xA5E0)
			Raylib.TraceLog(TraceLogLevel.LOG_ERROR, "invalid aseprite file, magic number is wrong");
		...
		for (int f = 0; f < Frames.Length; ++f)
		{
			...
			if (ReadWord() != 0xF1FA)
				Raylib.TraceLog(TraceLogLevel.LOG_ERROR, "frame magic number is incorrect");
